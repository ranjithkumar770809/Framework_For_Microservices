...
public class ProductController {
    ...
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<List<Product>>> searchProducts(@RequestParam(required = false, defaultValue = "") String keyword) {
    ...
    public ResponseEntity<ApiResponse<Product>> decrementStock(
        ...
        int qty = (quantity != null) ? quantity : (request != null && request.getQuantity() != null ? request.getQuantity() : 1);
        Product updatedProduct = productService.decrementStock(id, qty);
        return ResponseEntity.ok(ApiResponse.success("Stock decremented successfully", updatedProduct));
    }
    ...
    public ResponseEntity<ApiResponse<Product>> restoreStock(
        ...
        return ResponseEntity.ok(ApiResponse.success("Stock restored successfully", updatedProduct));
    }
}
